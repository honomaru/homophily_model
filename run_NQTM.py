...
def get_E(model):
    train_E = model.sess.run(model.topic_dis_quant._E)
    return train_E

def get_weights(model):#numはlayernum
    weights = model.sess.run(model.network_weights)
    return weights
...
def train(model, train_data, vocab, config):
    ...
    print_top_words(beta, vocab, config['n_top_words'])

    train_theta = get_theta(model, train_data)
    train_E = get_E(model)
    train_weights = get_weights(model)
    np.save(os.path.join(args.output_dir, 'theta_K{}_{}th'.format(args.topic_num, args.test_index)), train_theta)
    np.save(os.path.join(args.output_dir, 'E_K{}_{}th'.format(args.topic_num, args.test_index)), train_E)
    np.save(os.path.join(args.output_dir, 'weights_K{}_{}th'.format(args.topic_num, args.test_index)), train_weights)
